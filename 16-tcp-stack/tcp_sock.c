...
void tcp_sock_inc_ref_cnt(struct tcp_sock *tsk)
{
	tsk->ref_cnt += 1;
}
...
static int tcp_sock_set_sport(struct tcp_sock *tsk, u16 port)
{
	if ((port && tcp_port_in_use(port)) ||
			(!port && !(port = tcp_get_port()))) //Question
		return -1;
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	if (tcp_sock_set_sport(tsk, tcp_get_port()) < 0) {
        printf("set port failed: there's no \
                avaliable port being allocated.\n");
        return -1;
    }
    tsk->sk_dip = ntohl(skaddr->ip);
    tsk->sk_dport = ntohs(skaddr->port);

    //tcp_bind_hash(tsk); we have put this in tcp_sock_set_sport()
...
void tcp_sock_close(struct tcp_sock *tsk)
{
	...
	switch (tsk->state) {
        ...
        case TCP_CLOSE_WAIT:
            ...
            break;
        default:
        	printf("tcp_sock_close: unexpected tcp state %d\n", tsk->state);
    }
}
...
int tcp_sock_read(struct tcp_sock *tsk, char *buf, int len) {
  if (ring_buffer_empty(tsk->rcv_buf)) {
  	if (tsk->state == TCP_CLOSE_WAIT)
  	 	return -1;
  	printf("tcp_sock_read: sleep because empty rcv_buf\n");
...
int tcp_sock_write(struct tcp_sock *tsk, char *buf, int len){
	...
	while(sent_len < len){
		...
		int effective_snd_wnd = (int)tsk->snd_wnd - (int)(tsk->snd_nxt - tsk->snd_una); //todo
		printf("debug snd_wnd:%d snd_nxt:%d snd_una:%d effec:%d pkt_len:%d\n", tsk->snd_wnd, tsk->snd_nxt, tsk->snd_una, effective_snd_wnd, data_len);
		if (effective_snd_wnd < data_len) sleep_on(tsk->wait_send);
