...
stp_port_t* find_root_port(stp_t *stp) {
	stp_port_t *result = NULL;
	for (int i = 0; i<stp->nports; i++) {
		if (stp_port_is_designated(&stp->ports[i])) continue;
		if (result == NULL) result = &stp->ports[i];
		else {
			if (stp->ports[i].designated_root < result->designated_root) result = &stp->ports[i];
			else if (stp->ports[i].designated_root > result->designated_root) continue;
			else if (stp->ports[i].designated_cost < result->designated_cost) result = &stp->ports[i];
			else if (stp->ports[i].designated_cost > result->designated_cost) continue;
			else if (stp->ports[i].designated_switch < result->designated_switch) result = &stp->ports[i];
			else if (stp->ports[i].designated_switch > result->designated_switch) continue;
			else if (stp->ports[i].designated_port < result->designated_port) result = &stp->ports[i];
			else if (stp->ports[i].designated_port > result->designated_port) continue;
			//else log(INFO, "port equally comparing: impossible");
		}
	}
	return result;
}

bool is_config_higher(stp_port_t *p, struct stp_config *config) {
	if (ntohll(config->root_id) < p->designated_root) return true;
	else if (ntohll(config->root_id) > p->designated_root) return false;
	else if (ntohl(config->root_path_cost) < p->designated_cost) return true;
	else if (ntohl(config->root_path_cost) > p->designated_cost) return false;
	else if (ntohll(config->switch_id) < p->designated_switch) return true;
	else if (ntohll(config->switch_id) > p->designated_switch) return false;
	else if (ntohs(config->port_id) < p->designated_port) return true;
	else if (ntohs(config->port_id) > p->designated_port) return false;
	//else log(INFO, "config equally comparing: impossible");
	return false;
}
...
static void stp_handle_config_packet(stp_t *stp, stp_port_t *p,
		struct stp_config *config)
{
	if (is_config_higher(p, config)) {
		//Replace Port Config
		p->designated_root = ntohll(config->root_id);
		p->designated_switch = ntohll(config->switch_id);
		p->designated_port = ntohs(config->port_id);
		p->designated_cost = ntohl(config->root_path_cost);
		//Update STP node state
		stp_port_t* rootport = find_root_port(stp);
		if (rootport == NULL) {
			stp->root_port = NULL;
			stp->designated_root = stp->switch_id;
			stp->root_path_cost = 0;
		} else {
			stp->root_port = rootport;
			stp->designated_root = rootport->designated_root;
			stp->root_path_cost = rootport->designated_cost + rootport->path_cost;
		}

		//Update rest ports state
		stp_port_t* port_i;
		for (int i = 0; i < stp->nports; i++) {
			port_i = &stp->ports[i];
		...
		//Stop hello timer of this non-root switch
		//if (!stp_is_root_switch(stp))
			stp_stop_timer(&stp->hello_timer);
		//send new config from every Designated Port
		stp_send_config(stp);
	} else {
		stp_port_send_config(p); //maybe
	}
